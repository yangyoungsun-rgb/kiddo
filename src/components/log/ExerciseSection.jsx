    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-bold text-slate-800 flex items-center gap-2">
          <span className="text-xl">운</span> 운동 기록
        </h3>
        <button
      ...
      <div className="flex items-center justify-between bg-blue-50/50 p-4 rounded-xl border border-blue-100 mb-5">
        <div className="flex items-center gap-2">
          <span className="text-2xl">보</span>
          <span className="text-slate-700 font-bold">오늘 걸음 수</span>
        </div>
        <div className="flex items-center gap-2">
          <input
            type="number"
        ...
        {todayLog.exercise.map(exe => (
          <div key={exe.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border border-slate-100">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button
                onClick={() => setTodayLog({ ...todayLog, exercise: todayLog.exercise.filter(item => item.id !== exe.id) })}
      ...
      <div className="flex items-stretch gap-2">
        <input
          type="text"
          id="customExeName"
          placeholder="직접 활동 입력"
          className="min-w-0 flex-1 bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
        />
        <input
          type="number"
          id="customExeCal"
          placeholder="kcal"
          className="w-20 shrink-0 bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 text-right"
        />
        <button
          onClick={() => {
          ...
          className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-slate-800 font-bold text-white hover:bg-slate-700"
        >
          <Plus size={20} />
        </button>
      </div>
